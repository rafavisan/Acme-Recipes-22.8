...
@Service
public class ChefPartOfArtifactListService implements AbstractListService<Chef, PartOf> {
	...
	public Collection<PartOf> findMany(final Request<PartOf> request) {
		...
		result = this.repository.findManyArtifactByPublisedAndRecipeId(masterId);
	...
	public void unbind(final Request<PartOf> request, final PartOf entity, final Model model) {
		...
		request.unbind(entity, model, "quantity","unit","artifact.name");
	}
